...
async def main():
    ...
    async with stealth.use_async(async_playwright()) as p:
        ...
        # Test fetch from within browser context
        for lid, url in LISTINGS[:3]:
            print(f"Fetching {lid} via in-page fetch...")
            result = await page.evaluate(f"""async () => {{
                try {{
                    ...
                    return {{ status: r.status, length: text.length, snippet: text.substring(0, 300) }};
                }} catch(e) {{
